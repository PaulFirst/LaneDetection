...
class Stack(object):
    def __init__(self, args):
        ...
        backs = ["resnet", "ibn", "resnet152", "resnet101"]
        check = './ckpt'
        checks = ["herbrand.pth.tar", "ign85.12.pth.tar", "r152_85.20.pth.tar", "resnet101.pth.tar"]
        self.models = []
        self.nclass = 3
        self.M = len(backs)
        # define models
        for i in range(self.M):
            model = DeepLab(self.args, num_classes=self.nclass,
                            backbone=backs[i],
                            output_stride=16,
                            norm='gn',
                            freeze_bn=False)
